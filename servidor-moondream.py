...
def upload_image():
    ...
    try:
        ...
        return (
            jsonify({"message": "Imagen recibida y guardada", "path": image_path}),
            200,
        )
...
def query_balloon_colour():
    ...
    try:
        ...
        contador_local = int(
            time.time() * 1000
        )  # timestamp en ms para evitar colisiones
        ...
        image = Image.open(
            image_file.stream
        )  # Utilizamos .stream para trabajar con la imagen en memoria
        ...
        answer = model.query(
            image, "Identify the different colors of any balloons you see."
        )["answer"]
        ...
        return (
            jsonify({"message": "Consulta procesada con éxito", **response_data}),
            200,
        )
